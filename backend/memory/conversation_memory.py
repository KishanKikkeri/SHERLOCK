...
class ConversationMemoryService:
    ...
    def get_all_findings(self, session_id: int) -> list[dict]:
        """Aggregate all findings across all conversation turns for a session."""
        turns = (
            self.session.query(ConversationTurn)
        ...
        for t in turns:
            if t.findings_json:
                try:
                    data = json.loads(t.findings_json)
        ...
        # Also check MessageV2 rows linked to conversation threads for this session/investigation
        try:
            from backend.database.models.conversation_v2 import ConversationV2, MessageV2
            convs = self.session.query(ConversationV2).filter_by(investigation_id=session_id, is_deleted=False).all()
            conv_ids = [c.id for c in convs]
            if conv_ids:
                msgs = self.session.query(MessageV2).filter(MessageV2.conversation_id.in_(conv_ids)).all()
                for m in msgs:
                    if m.tool_result_json:
                        try:
                            res = json.loads(m.tool_result_json)
                            items = res.get("results") or res.get("findings") or res.get("data")
