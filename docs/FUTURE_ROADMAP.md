...
**Sociological Intelligence Agent** *(Agent 2 workstream)* — implemented against what the schema actually has: demographic breakdown (accused + victims), occupation-crime correlation, and social risk factors (repeat-offender FIR tallies, family links via `PersonAssociation`, gang indicators via real `Organization`/`OrganizationMembership` data, crime-density-by-district as a vulnerability proxy). Income, employment status, housing type, economic category, urbanization, migration, and education are still genuinely absent from the schema — each is reported as unavailable rather than estimated, with a real extension-point method already built on `SociologicalInsightsService` (backend/intelligence/sociological_insights.py) so wiring in a real dataset later needs no pipeline changes. See `GET /analytics/sociological` and `/analytics/sociological/report`, and the dedicated frontend dashboard at `/analytics/sociological`.
