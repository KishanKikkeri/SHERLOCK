import type{AgentStep,FeedEntry}from'../../lib/types';
...
const FailedIcon=()=><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9"/><path d="M9.5 9.5l5 5m0-5l-5 5" strokeLinecap="round"/></svg>;
function fmt(ms:number){return ms<1000?`${ms}ms`:`${(ms/1000).toFixed(1)}s`;}
export function InvestigationTimeline({steps,elapsedMs,feed}:{steps:AgentStep[];elapsedMs:number;feed:FeedEntry[]}){
    ...
    <nav className={s.steps}>
      {steps.map((step,i)=>(
        <div key={step.nodeKey} className={`${s.step} ${s[step.status]}`} aria-current={step.status==='running'?'step':undefined}>
          ...
          <div className={s.icon} aria-hidden>{step.status==='pending'?<PendingIcon/>:step.status==='running'?<RunningIcon/>:step.status==='complete'?<DoneIcon/>:step.status==='failed'?<FailedIcon/>:<SkipIcon/>}</div>
          <div className={s.content}>
            ...
            {step.status==='failed'&&<span className={s.failMsg} title={step.message??undefined}>{(step.message??'Failed unexpectedly').slice(0,60)}</span>}
          </div>
        </div>
      ))}
    </nav>
