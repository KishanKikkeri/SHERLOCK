...
export function FindingsPanel({findings,isComplete,onExportPDF,exportingPDF,narrative,voice}:{findings:AgentFinding[];isComplete:boolean;onExportPDF:()=>void;exportingPDF:boolean;narrative:string|null;voice:ReturnType<typeof useVoice>}){
  ...
  return(<aside className={s.root} aria-label="Findings">
    ...
    {isComplete&&<div className={s.exportWrap}>
      {narrative&&voice.state.supported&&<button className={s.speakBtn} onClick={()=>voice.state.speaking?voice.actions.cancelSpeech():voice.actions.speak(narrative)}>
        {voice.state.speaking?<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="6" y="5" width="4" height="14" rx="1"/><rect x="14" y="5" width="4" height="14" rx="1"/></svg>:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M11 5 6 9H2v6h4l5 4V5zM16.5 8.5a5 5 0 0 1 0 7M19.5 5.5a9 9 0 0 1 0 13"/></svg>}
        {voice.state.speaking?'Stop reading':'Read summary aloud'}
      </button>}
      <button className={s.exportBtn} onClick={onExportPDF} disabled={exportingPDF}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M7 10l5 5 5-5M12 15V3"/></svg>{exportingPDF?'Exporting…':'Export Investigation Report'}</button>
    </div>}
  </aside>);
}
