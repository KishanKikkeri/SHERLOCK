...
export function NLConsole({onSubmit,isRunning,lastQuery,voice}:{onSubmit:(q:string,viaVoice?:boolean)=>void;isRunning:boolean;lastQuery:string;voice:ReturnType<typeof useVoice>}){
  ...
  const submit=useCallback((text?:string,viaVoice?:boolean)=>{const q=(text??query).trim();if(!q||isRunning)return;setHIdx(-1);onSubmit(q,viaVoice);setQuery('');},[query,isRunning,onSubmit]);
  ...
  return(<div className={s.root} role="search"><div className={s.inner}>
    ...
    <input ref={ref} className={s.input} placeholder={voice.state.dictating?(voice.state.transcript||'Listening…'):isRunning?'Investigation in progress…':'Ask a follow-up or start a new investigation…'} value={voice.state.dictating?'':query} onChange={e=>setQuery(e.target.value)} onKeyDown={keyDown} disabled={isRunning} aria-label="Investigation query" autoComplete="off" spellCheck={false}/>
    <div className={s.controls}>
      ...
      {voice.state.supported&&<>
        <button
          className={`${s.voiceBtn} ${voice.state.wakeListening?s.voiceActive:''}`}
          onClick={voice.actions.toggleWakeListening}
          disabled={isRunning}
          title={voice.state.wakeListening?'Stop listening for "Sherlock"':'Listen for wake word "Sherlock"'}
          aria-pressed={voice.state.wakeListening}
        >👂</button>
        <button
          className={`${s.voiceBtn} ${voice.state.dictating?s.voiceActive:''}`}
          onPointerDown={voice.actions.startPushToTalk}
          onPointerUp={voice.actions.stopPushToTalk}
          onPointerLeave={()=>{if(voice.state.dictating)voice.actions.stopPushToTalk();}}
          disabled={isRunning}
          title="Hold to talk"
          aria-pressed={voice.state.dictating}
        >🎙</button>
        {voice.state.speaking&&<button className={`${s.voiceBtn} ${s.voiceActive}`} onClick={voice.actions.cancelSpeech} title="Stop speaking">🔊</button>}
      </>}
